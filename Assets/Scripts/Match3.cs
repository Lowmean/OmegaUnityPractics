...
public class Match3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < finishedUpdating.Count; i++)
        {
            ...
            if (connected.Count == 0) //No match
            {
                if (wasFlipped) //Moving animation 
                    FlipPieces(piece.index, flippedPiece.index, false); //If we dont get match 
            }
            else //Match
            {
    ...
    public void ApplyGravityToBoard() 
    {
        ...
        {
            for (int y = (height - 1); y >= 0; y--) //Start from the bottom of the board 
            {
    ...
    List<Point> isConnected(Point p, bool main)
    {
        ...
        foreach(Point dir in directions) //Checking if there is 2 or more same shapes in the directions
        {
            ...
            if (same > 1) //If there are more than 1 of the same elements in the direction then we know it is a match
                AddPoints(ref connected, line); //Add these points to the overarching connected list
        }

        for(int i = 0; i < 2; i++) //Checking if we are in the middle of two of the same elements
        {
            List<Point> line = new List<Point>();
...
public class Node
{
    public int value; //0 = pusto, 1 = buterbrod, 2 = IceCream, 3 = Juice, 4 = Juice2, 5 = Juice3, -1 = HoleInTheBoard
    public Point index;
    NodePiece piece;
